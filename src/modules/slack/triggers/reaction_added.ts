...
module.exports = {
    spec: {
        id: 'reaction_added',
        pretty_name: 'New Reaction Added',
        description: 'Triggered when a reaction is added to a message or item.',
        webhook: {
            method: 'POST',
            path: '/webhook/slack/reactions/:workflowId'
        },
        options: [
    ...
    register: function (_app: any, wf: any, trig: any, actionsList: any[], registry: any, options: any, registrars?: any) {
        ...
        const routePath = (trig && trig.path) || '/webhook/slack/reactions/' + wf.id;

        router.post('/', async (req: any, res: any) => {
            const secret = (trig && trig.signing_secret) || '';
            ...
            const triggerOutputs = { body: payload, event: event, headers: req.headers, params: req.params, query: req.query };
            ...
            try {
                ...
                console.error('slack reaction trigger error', e);
