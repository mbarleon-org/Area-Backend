...
async function sheetsAppend(accessToken: string, spreadsheetId: string, range: string, values: any, valueInputOption: string) {
    const fetchFn = getFetch();
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(spreadsheetId)}/values/${encodeURIComponent(range)}:append?valueInputOption=${encodeURIComponent(valueInputOption)}`;
    const body = {
    ...
    if (!res.ok || json.error) {
        const err = (json && json.error && json.error.message) ? json.error.message : 'sheets_append_failed';
        throw new Error(err);
    }
    return json;
}
...
module.exports = {
    ...
    handler: async (ctx: any, inputs: Record<string, any>) => {
        ...
        const res = await sheetsAppend(token, inputs.spreadsheet_id, inputs.range, inputs.values, valueInputOption);
