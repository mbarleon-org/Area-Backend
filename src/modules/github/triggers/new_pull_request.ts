...
module.exports = {
    spec: {
        id: 'new_pull_request',
        pretty_name: 'New Pull Request Opened',
        description: 'Trigger when a new GitHub pull request is opened.',
        webhook: {
            method: 'POST',
            path: '/webhook/github/prs/:workflowId'
        },
        options: [
        ...
        outputs: [
            ...
            { id: 'pull_request', pretty_name: 'Pull Request', type: 'json' },
            { id: 'headers', pretty_name: 'Headers', type: 'json' }
        ]
    },

    handler: async function (_ctx: any, inputs: Record<string, any>) {
        return { body: inputs.body || null, pull_request: inputs.pull_request || null, headers: inputs.headers || null };
    },

    register: function (_app: any, wf: any, trig: any, actionsList: any[], registry: any, options: any, registrars?: any) {
        const router = express.Router();
        const routePath = (trig && trig.path) || '/webhook/github/prs/' + wf.id;
        ...
        router.post('/', async (req: any, res: any) => {
            ...
            if (eventType !== 'pull_request') return res.status(202).json({ ok: true, ignored: true });
            const payload = req.body || {};
            if (!payload.action || payload.action !== 'opened') return res.status(202).json({ ok: true, ignored: true });

            const pr = payload.pull_request || {};
            const triggerOutputs = { body: payload, pull_request: pr, headers: req.headers, params: req.params, query: req.query };
            ...
            try {
                ...
                console.error('github new_pull_request trigger error', e);
